...
{
	ref class SymbolicDiffDTW : public Operation {
	private:
		unsigned int Order;
		String ^name;
		double threshold;
	...
	public:
		...
		virtual array<array<float>^> ^ PreprocessActionTemplate(array<array<float> ^> ^p_saved_action_definition, array<int> ^p_sdi)
		{
			...
			return SymbolicDiffDTW::Symbolize(p_saved_action_definition, p_sdi, 4);
		}
		...
		virtual array<double>^ Apply(array<array<float> ^> ^p_saved_action_definition, array<array<float> ^> ^p_last_actions, array<int> ^p_data_offsets) override
		{
			...
			p_last_actions = SymbolicDiffDTW::Normalize(p_last_actions, p_data_offsets, -1, 1);

			array<array<float>^> ^symbolic_list = SymbolicDiffDTW::Symbolize(p_last_actions, p_data_offsets, 4);
