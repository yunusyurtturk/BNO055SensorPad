		...
		*/
		...
		for (int sdi = 0; sdi < p_data_offsets->Length; sdi++) {
			...
			while (i >= 0 && j >= 0) {
				...
				double current_diff = Math::Abs(p_saved_action_definition[j][sensor_data_index_of_saved] - p_last_actions[i][sensor_data_index_of_last]);
				if (0.001 < Math::Abs(current_diff)){

					if (Math::Abs(p_saved_action_definition[j][sensor_data_index_of_saved]) > 1.5 || Math::Abs(p_last_actions[i][sensor_data_index_of_last]) > 1.5) {
				...
				dtw_distance = dtw_distance + current_diff;
			...
			if (FileSave) {
				SaveDirIndex++;
				String ^dir = gcnew String(Environment::CurrentDirectory + "\\SensorData\\Signals\\" + SaveDirIndex.ToString() + "DTWSax");
